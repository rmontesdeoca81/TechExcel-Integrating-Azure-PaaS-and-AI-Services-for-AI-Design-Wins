...
#pragma warning restore SKEXP0001 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
        ...
        /// </summary>
        public async Task<List<VectorSearchResult>> ExecuteVectorSearch(float[] queryVector, int maxResults = 0, double minimumSimilarityScore = 0.8)
        {
            ...
            var query = $"SELECT c.hotel_id AS HotelId, c.hotel AS Hotel, c.details AS Details, c.source AS Source, VECTOR_DISTANCE(c.request_vector, [{vectorString}]) AS SimilarityScore FROM c";
            query += $" WHERE VECTOR_DISTANCE(c.request_vector, [{vectorString}]) > {minimumSimilarityScore.ToString(CultureInfo.InvariantCulture)}";
            query += $" ORDER BY VECTOR_DISTANCE(c.request_vector, [{vectorString}])";
